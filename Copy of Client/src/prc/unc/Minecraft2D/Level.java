...
public class Level extends DoubleRectangle {
	...
	public Character[][] character = new Character[worldW][worldH];
	public Mob[][] mob = new Mob[worldW][worldH];
	public Block2[][] block2 = new Block2[worldW][worldH];
	public Block[][] block = new Block[worldW][worldH];
	public Light[][] light = new Light[worldW][worldH];
	...
	public Level() {
		...
		for (int x = 0; x < block2.length; x++) {
			for (int y = 0; y < block[0].length; y++) {
				block2[x][y] = new Block2(new Rectangle(x * Tile.tileSize, y * Tile.tileSize, Tile.tileSize, Tile.tileSize), Tile.air);
			}
		}
					...
					if (x >= 0 && y >= 0 && x < worldW && y < worldH) {
						if (block[x][y].contains(new Point((Component.mse.x / Component.pixelSize) + (int) Component.sX, (Component.mse.y / Component.pixelSize) + (int) Component.sY))) {
							...
							if (Component.isMouseLeft && !Inventory.isOpen) {
								if (block[x][y].id != Tile.solidair && block[x][y].id != Tile.air) {
									block[x][y].id = Tile.air;
									bx++;
