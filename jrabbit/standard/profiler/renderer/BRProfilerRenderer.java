...
/*****************************************************************************
 * BRProfilerRenderer (short for "Bottom-Right" ProfilerRenderer) renders the 
 * list of ProfilerEntities from the bottom-right corner of the screen, going 
 * up. Every time the rendering hits the top of the allowed area, the next 
 * ProfilerEntity is rendered at the bottom of the area and to the left. 
 * 
 * @author Chris Molini
 *****************************************************************************/
public class BRProfilerRenderer extends ProfilerRenderer
{
	/*************************************************************************
	...
	/*************************************************************************
	 * Renders the indicated list of ProfilerEntities from the bottom-right 
	 * corner of the screen. Rendering heads towards the top-left of the screen.
	 * 
	 * @param entities
	...
	{
		GL11.glTranslatef(width, height, 0);
		float shiftY = 0;
		float shiftX = 0;
		for(ProfilerEntity entity : entities)
			if(entity.visible())
			{
				...
				GL11.glTranslatef(-frameWidth, -frameHeight, 0);
				color.bind();
				entity.render();
				color.release();
				GL11.glTranslatef(frameWidth, -padding, 0);
				shiftY += height + padding;
